...
Timeline.prototype.makeLabel = function (time) {
  ...
  //   gregorian.second
  // )}${millisecondString}`;
  var jsdate = JulianDate.toDate(time);

  return jsdate.getFullYear() + '年' + (jsdate.getMonth() + 1) + '月' + jsdate.getDate() + '日' + twoDigits(jsdate.getHours()) +
         ':' + twoDigits(jsdate.getMinutes()) + ':' + twoDigits(jsdate.getSeconds());
  //【世纪空间 ATGlobe】转本地时间
};
